...
def calculate_crypto_kpis():
    ...
    try:
        ...
        sentiment = "Neutral"
        if prices:
            avg_change = sum(float(p.get("price_change_24h") or 0) for p in prices) / len(prices)
            if avg_change > 1:
                sentiment = "Bullish"
            elif avg_change < -1:
                sentiment = "Bearish"

        return {"prices": prices if prices else [], "daily_stats": stats if stats else [], "market_sentiment": sentiment}
...
def calculate_stock_kpis():
    ...
    try:
        ...
        return {"daily_changes": stocks if stocks else [], "sector_performance": sector_perf}
    except Exception as e:
